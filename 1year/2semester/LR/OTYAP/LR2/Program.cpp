...
const state state_table[4][8] = 
{
    ...
    { s_begin,          s_deadend, s_deadend, s_end,    s_end,   s_end, s_deadend,   s_deadend },
...
vector<unsigned char*> is_alternating_digits_letters(unsigned char* text)
{
    vector<unsigned char*> lexems;
    ...
    do
    {
        ...
        if (is_final_state(curr_state))
        {
            if (curr_state == s_end)
            {
                ...
                unsigned char* lexem = new unsigned char[length + 1];

                memcpy(lexem, text + lex_begin_i, length);
...
void output_tokens(vector<unsigned char*>& lexems)
{
...
int main()
{
    ...
    vector<unsigned char*> lexems = is_alternating_digits_letters(content);

...
class search_tree {
    private:
        ...
        /// @tparam F 
        /// @param node текцщий узел с которого нужно продолжить итерирование
        /// @param func функция принимающая два узла - текущий узел и его родитель. возвращающая true если нужно продолжить итерирование, false иначе.
        /// @param parent родитель текущего узла
        /// @return true если функция выполнилась полностью и не была прервана, false иначе
        template<typename F>
        bool foreach_inorder_node(tree_node<T>* node, F && func, tree_node<T>* parent = NULL)
        {
            if (!node) return true;

            if (node->get_left())
            {
                if(!foreach_inorder_node(node->get_left(), func, node))
                {
                    return false;
                }
            }
            ...
            if (right)
            {
                if(!foreach_inorder_node(right, func, node))
                {
                    return false;
                }
            }
    ...
    public:
        ...
        {
            foreach_inorder_node(root, [&func](tree_node<T>* n, tree_node<T>* p) { func(n->get_data()); return true; } );
        }

        ~ search_tree()
        {
            //удаляем все узлы
            foreach_inorder_node(root, [](tree_node<T>* curr, tree_node<T>* p) { delete curr; return true; });
        }
};
